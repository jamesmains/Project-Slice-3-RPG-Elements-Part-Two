...
namespace CMF
{
	...
	public class AnimationControl : MonoBehaviour {
		...
		void Update () {
			...
			Debugger.AddField($"Horizontal: {_horizontalVelocity.ToString()}", "Raw Velocity");
			Debugger.AddField($"Vertical: {_verticalVelocity.ToString()}", "Raw Velocity");
			...
			if(useStrafeAnimations)
			{
				...
				Debugger.AddField($"Vel: {_localVelocity.ToString()}", "Local Velocity");
				animator.SetFloat("ForwardSpeed", _localVelocity.z);
				animator.SetFloat("StrafeSpeed", _localVelocity.x);
			}
			
			Debugger.AddField($"Grounded ? {controller.IsGrounded().ToString()}", "State");
