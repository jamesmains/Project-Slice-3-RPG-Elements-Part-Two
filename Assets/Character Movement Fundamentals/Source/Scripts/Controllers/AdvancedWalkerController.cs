...
namespace CMF
{
	...
	public class AdvancedWalkerController : Controller {
        ...
        {
            bool _newJumpKeyPressedState = characterInput != null && characterInput.IsJumpKeyPressed();
		...
		protected virtual Vector3 CalculateMovementDirection()
		{
			...
			Vector3 _velocity = Vector3.zero;
			var _playerInput = characterInput.GetPlayerMoveInput();
			
			//If no camera transform has been assigned, use the character's transform axes to calculate the movement direction;
			if(cameraTransform == null)
			{
				_velocity += tr.right * _playerInput.x;
				_velocity += tr.forward * _playerInput.y;
			}
			else
			{
				//If a camera transform has been assigned, use the assigned transform's axes for movement direction;
				//Project movement direction so movement stays parallel to the ground;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.right, tr.up).normalized * _playerInput.x;
				_velocity += Vector3.ProjectOnPlane(cameraTransform.forward, tr.up).normalized * _playerInput.y;
			}
